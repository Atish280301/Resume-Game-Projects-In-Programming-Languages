...
void snake() { //create of snake
	...
	x = row/2, y = col/2, head = 3, tail = 1, Gy = y, game = 0, fruit = 0, score = 0, speed = 100;
...
void print() { //boundary
	...
	for(row = 0; row < N; row++) {
		...
		for(col = 0; col < M; col++) {
			if(field[row][col] == 0)
				printf(" ");
			if(field[row][col] > 0 && field[row][col] != head)//Snake Body Creation
				printf("0");
			if(field[row][col] == head)//Snake Head
				printf("O");
			if(field[row][col] == -1) //Generate Fruit
				printf("@");
			if(col == M-1)
				printf("%c\n",186);
		}
...
void resetscreen() { //reset the entire screen
	HANDLE hOut;
	COORD Position;
...
void movement() {	//movement of Snake
	variable = getchar();
